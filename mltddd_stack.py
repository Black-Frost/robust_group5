...
if side == 'send':
  ...
  server_instance = [socket.socket(socket.AF_INET, socket.SOCK_DGRAM) for _ in range(threads)]
  for thread in range(threads):
    server_instance[thread].bind((bind_host, bind_port + thread))
